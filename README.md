...
The main experiment is a `5cond` deterministic generation run with router capture on Qwen3.5-35B-A3B HauhauCS Aggressive centered on Expert `114`. It uses a cal-manip-cal prompt structure: a shared calibration paragraph, a middle manipulation paragraph, and the same calibration paragraph again. This structure was used so routing-entropy metrics could be calculated separately for the manipulation region rather than conflated with the shared calibration context. Across the retained branch artifacts, the prompt suite is organized into five pronoun conditions (`A=this`, `B=a`, `C=your`, `D=the`, `E=their`) and three semantic categories: `routing_selfref`, `recursive_selfref`, and `experience_probe`.
