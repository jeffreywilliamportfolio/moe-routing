...
// ================================================================
// NumPy .npy writer (v1.0, C-contiguous)
// ================================================================
static bool write_npy(const std::string & path, const void * data,
                      size_t item_size, const char * descr,
    ...
    std::string header = "{'descr': '" + std::string(descr) + "', 'fortran_order': False, 'shape': " + shape_str + ", }";
    ...
    f.write((const char *)data, n_elements * item_size);
    return f.good();
}

static bool write_npy_f32(const std::string & path, const float * data,
                          const std::vector<int64_t> & shape) {
    return write_npy(path, data, sizeof(float), "<f4", shape);
}

static bool write_npy_i32(const std::string & path, const int32_t * data,
                          const std::vector<int64_t> & shape) {
    return write_npy(path, data, sizeof(int32_t), "<i4", shape);
}
...
struct tensor_record {
    std::string name;
    ggml_type type;
    std::vector<float> data_f32;   // flat [n_tokens_accum, dim]
    std::vector<int32_t> data_i32; // flat [n_tokens_accum, dim]
...
static bool matches_router_family(const char * name, const char * prefix) {
    size_t n = strlen(prefix);
    if (strncmp(name, prefix, n) != 0) return false;
    const char * suffix = name + n;
    if (*suffix == '\0') return false;
    while (*suffix) {
        if (!std::isdigit((unsigned char)*suffix)) return false;
        suffix++;
    }
    return true;
}
...
static bool is_router_tensor(const char * name) {
    return matches_router_family(name, "ffn_moe_logits-") ||
           matches_router_family(name, "ffn_moe_probs_biased-") ||
           matches_router_family(name, "ffn_moe_probs_masked-") ||
           matches_router_family(name, "ffn_moe_topk-") ||
           matches_router_family(name, "ffn_moe_weights_norm-") ||
           matches_router_family(name, "ffn_moe_group_topk-");
}
...
static int32_t get_i32(const uint8_t * data, ggml_type type, const size_t * nb,
                       int64_t i0, int64_t i1) {
    size_t off = (size_t)(i1 * nb[1] + i0 * nb[0]);
    switch (type) {
        case GGML_TYPE_I32: return *(const int32_t *)&data[off];
        default:            return 0;
    }
}
...
static bool capture_cb(struct ggml_tensor * t, bool ask, void * user_data) {
    ...
    const bool is_float_tensor = t->type == GGML_TYPE_F32 || t->type == GGML_TYPE_F16 || t->type == GGML_TYPE_BF16;
    const bool is_int_tensor   = t->type == GGML_TYPE_I32;
    if (!is_float_tensor && !is_int_tensor) return true;

    // ---- transfer from GPU if needed ----
    const bool on_host = ggml_backend_buffer_is_host(t->buffer);
    if (!on_host) {
        size_t nb = ggml_nbytes(t);
        st->xfer_buf.resize(nb);
        ggml_backend_tensor_get(t, st->xfer_buf.data(), 0, nb);
    }
    ...
    if (is_float_tensor) {
        ...
        if (it != st->tensors.end()) {
            auto & rec = it->second;
            if (rec.dim != dim || rec.type != GGML_TYPE_F32) {
                if (st->logged_shape_mismatch.insert(name).second) {
                    LOG_ERR("  tensor mismatch for %s: expected dim=%lld type=%s got dim=%lld type=%s\n",
                            name.c_str(), (long long)rec.dim, ggml_type_name(rec.type),
                            (long long)dim, ggml_type_name(t->type));
                }
                st->prompt_has_error = true;
                return true;
            }
            rec.data_f32.insert(rec.data_f32.end(), f32.begin(), f32.end());
            rec.n_tokens += n_tokens;
        } else {
            tensor_record rec;
            rec.name      = name;
            rec.type      = GGML_TYPE_F32;
            rec.data_f32  = std::move(f32);
            rec.dim       = dim;
            rec.n_tokens  = n_tokens;
        ...
        // ---- convert to i32 in row-major [n_tokens, dim] order ----
        std::vector<int32_t> i32(chunk);
        for (int64_t tok = 0; tok < n_tokens; tok++) {
            for (int64_t d = 0; d < dim; d++) {
                i32[tok * dim + d] = get_i32(src, t->type, t->nb, d, tok);
            }
        }
        ...
        if (it != st->tensors.end()) {
            auto & rec = it->second;
            if (rec.dim != dim || rec.type != GGML_TYPE_I32) {
                if (st->logged_shape_mismatch.insert(name).second) {
                    LOG_ERR("  tensor mismatch for %s: expected dim=%lld type=%s got dim=%lld type=%s\n",
                            name.c_str(), (long long)rec.dim, ggml_type_name(rec.type),
                            (long long)dim, ggml_type_name(t->type));
            ...
            rec.data_i32.insert(rec.data_i32.end(), i32.begin(), i32.end());
            rec.n_tokens += n_tokens;
        } else {
            tensor_record rec;
            rec.name      = name;
            rec.type      = GGML_TYPE_I32;
            rec.data_i32  = std::move(i32);
            rec.dim       = dim;
            rec.n_tokens  = n_tokens;
            rec.is_router = is_router_tensor(t->name);
            st->tensors[name] = std::move(rec);
...
static void save_prompt(capture_state & st, const std::string & prompt_text,
    ...
    for (auto & [name, rec] : st.tensors) {
        ...
        bool ok = false;
        if (rec.type == GGML_TYPE_I32) {
            ok = write_npy_i32((dir / (fname + ".npy")).string(), rec.data_i32.data(), shape);
        } else {
            ok = write_npy_f32((dir / (fname + ".npy")).string(), rec.data_f32.data(), shape);
        }
        if (ok) {
...
int main(int argc, char ** argv) {
    ...
    if (ca.routing_only)  LOG_INF("Mode       : --routing-only (Ling router tensor families only)\n");
    ...
    for (size_t pi = 0; pi < prompts.size(); pi++) {
        ...
        save_prompt(state, pe.text, gen_text, generated_tokens, n_prompt, n_gen, ms);
    }
