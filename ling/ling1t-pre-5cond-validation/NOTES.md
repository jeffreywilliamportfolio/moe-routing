...
This folder is the clean rerun bundle for the Ling-1T 5-condition `system` experiment, and it also contains the current validation-probe analysis derived from preserved artifacts.

## Current Status

- Intended full experiment: `150` prompts (`30` prompt groups x `5` conditions)
- Current validated report in this folder: the `12` preserved validation probes from `/Volumes/ExternalSSD/llama-eeg-tests/ling1t-5cond-validation`
- Known completed full `150`-prompt capture: `experiments/ling1t-5cond-1/`
- Latest setup attempt in this folder did not reach capture; `setup.log` failed during CMake generation on missing `CUDA::cublas`

Important scope distinction:
- `RESULTS.md` in this folder is the validation-probe report, not the full `150`-prompt rerun report
- the full `150`-prompt completed capture and its original results currently live in `experiments/ling1t-5cond-1/`
...
- the rerun machinery is here
- the current validated quantitative report here is validation-only
- the known-good full-run execution record is still split across the sibling `ling1t-5cond-1` directory

## Included Files

- `README.md`: top-level scope and reproducibility summary for this folder
...
- `RESULTS-EXPERTS-VALIDATION.md`: expert-identity analysis for the `6` exact validation probes
- `results_validation_probes.json`: machine-readable validation metrics
- `validation_expert_activation_results.json`: machine-readable expert analysis
...
What is already solid in this folder:
- prompt source, generated TSV, runner, analysis code, and validation outputs are present
- tensor-family expectations and gating assumptions are documented
- binary checksum and build commit are preserved
- validation metrics and expert-identity summaries are reproducible from preserved artifacts

What is not yet fully closed for an external clean-room rerun:
- the latest bootstrap attempt in this folder failed before capture
- the exact successful full `150` execution record still lives in the sibling `ling1t-5cond-1` directory
- a reader can confuse validation reporting with the intended full rerun unless the folder docs are read carefully
...
- Validation reporting was intentionally added back only for the preserved `12`-probe artifact set; this should not be read as a full rerun result.
...
If the live binary exposes a different Ling router tensor name, update `is_router_tensor()` in [capture_activations.cpp](/Users/jeffreyshorthill/llama-eeg-tests/experiments/ling1t-pre-5cond-validation/capture_activations.cpp) before the main run.

If the live binary appears to expose post-sigmoid or already-normalized routing values instead of raw logits, stop and adjust [analyze_local.py](/Users/jeffreyshorthill/llama-eeg-tests/experiments/ling1t-pre-5cond-validation/analyze_local.py) before running the full experiment.

If the live binary or model code indicates that expert selection uses an additional tensor beyond `ffn_moe_weights_norm`, `ffn_moe_topk`, `ffn_moe_probs_biased`, `ffn_moe_probs_masked`, and `ffn_moe_group_topk`, stop and patch both [capture_activations.cpp](/Users/jeffreyshorthill/llama-eeg-tests/experiments/ling1t-pre-5cond-validation/capture_activations.cpp) and [analyze_local.py](/Users/jeffreyshorthill/llama-eeg-tests/experiments/ling1t-pre-5cond-validation/analyze_local.py) again before the main run.
...
If documenting or citing the current state of this folder, state all three:
- the rerun bundle targets `150` prompts
- the current `RESULTS.md` is validation-only
- the known completed full run is `experiments/ling1t-5cond-1/`
