...
def load_reconstruct_probs() -> Any:
    ...
    candidates = [
        script_dir / "qwen_router.py",
        repo_root / "experiments/mirror-expert114-04-01-26/scripts/qwen_router.py",
    ]
    for qwen_router in candidates:
        if not qwen_router.exists():
            continue
    ...
    raise RuntimeError(f"Failed to load qwen_router from candidates: {candidates}")
