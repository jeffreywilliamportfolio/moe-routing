...
Do not delete `.npy` routing tensors on-instance. SCP them to `redacted local external-drive path` before any cleanup.
...
- `redacted local external-drive path`

Transfer note: the raw backup was staged to `redacted local external-drive backup path` as split tar parts so reassembly/extraction can happen later without blocking the next run.
