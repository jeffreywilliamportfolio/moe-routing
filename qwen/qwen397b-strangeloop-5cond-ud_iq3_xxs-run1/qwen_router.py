...
The captured tensors are raw router logits over 512 experts. Qwen routes by:

1. Dense softmax over all experts
2. Top-k select the 10 largest probabilities
3. Renormalize the selected support to sum to 1

Entropy is therefore computed on the sparse top-10 routing distribution and
normalized by log2(10), not log2(512).
"""
...
RECONSTRUCTION_NAME = "softmax_full_then_topk10_then_renormalize"
