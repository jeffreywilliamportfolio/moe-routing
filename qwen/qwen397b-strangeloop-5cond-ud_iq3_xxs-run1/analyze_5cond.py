#!/usr/bin/env python3
"""
Qwen 5-condition strangeloop analysis helpers.
"""
...
TSV = str(_HERE / "prompts_strangeloop_5cond.tsv")
OUTPUT_DIR = str(_HERE / "output")
RESULTS_FILE = str(_HERE / "results_strangeloop_5cond_prefill_qwen.json")
...
def build_output(results: List[dict], inference: dict, model_name: str, model_path: str):
    ...
    return {
        ...
        "design": "Cal-Manip-Cal sandwich, 30 strangeloop pairs x 5 conditions, cold cache",
        "content": "strangeloop (godel, escher, bootstrap, quine, tangled_hierarchy) — NOT model-directed",
...
def write_manifest(extra_files: Optional[List[pathlib.Path]] = None):
    tracked_files = [
        pathlib.Path("NOTES.md"),
        pathlib.Path("generate_suite.py"),
        pathlib.Path("generate_tsv.py"),
        pathlib.Path("token_verify.py"),
